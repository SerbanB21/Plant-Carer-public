...
bool forcedPumpState = false;
bool isPumpRunning = false;
bool lastOverrideState = false;
...
void loop() {
  ...
  bool overrideState = (digitalRead(BTN_OVERRIDE) == LOW);
  ...
  if (overrideState && !lastOverrideState) {
    ...
    if (overrideActive) {
      forcedPumpState = !isPumpRunning; 
    }
    
    display.invertDisplay(true); delay(20); display.invertDisplay(false);
  ...
  if (now - lastReadTime >= 1000) {
    ...
    if (overrideActive) {
      isPumpRunning = forcedPumpState;
    } else {
      isPumpRunning = (currentSoil < pumpThreshold);
    }

    if (isPumpRunning) {
      analogWrite(PUMP_IN1, 127);  
      digitalWrite(PUMP_IN2, LOW); 
    } else {
      analogWrite(PUMP_IN1, 0);    
      digitalWrite(PUMP_IN2, LOW);
...
void playWarningMelody() {
  ...
  if (soundType == 0) { 
    if (beepState == 0) {
      tone(speakerPin, 659); 
      if (now - lastBeepTime > 150) { lastBeepTime = now; beepState = 1; }
    } else if (beepState == 1) {
      noTone(speakerPin); 
      if (now - lastBeepTime > 50) { lastBeepTime = now; beepState = 2; }
    } else if (beepState == 2) {
      tone(speakerPin, 523); 
      if (now - lastBeepTime > 200) { lastBeepTime = now; beepState = 3; }
    } else if (beepState == 3) {
      noTone(speakerPin); 
      if (now - lastBeepTime > 1500) { lastBeepTime = now; beepState = 0; } 
    }
  } 
  else if (soundType == 1) {
    if (beepState == 0) {
      tone(speakerPin, 880); 
      if (now - lastBeepTime > 100) { lastBeepTime = now; beepState = 1; }
    } else if (beepState == 1) {
      noTone(speakerPin);
      if (now - lastBeepTime > 100) { lastBeepTime = now; beepState = 0; }
    }
  }
  else if (soundType == 2) {
    if (beepState == 0) {
      tone(speakerPin, 600); 
      if (now - lastBeepTime > 400) { lastBeepTime = now; beepState = 1; }
    } else if (beepState == 1) {
      tone(speakerPin, 400);
      if (now - lastBeepTime > 400) { lastBeepTime = now; beepState = 0; }
    }
  }
}
...
void handleLeftPress() {
  ...
  if (currentScreen == 4) {
    ...
    else { currentSettingPage--; if (currentSettingPage < 0) currentSettingPage = 13; } 
    refreshScreen();
  }
}
...
void handleRightPress() {
  ...
  if (currentScreen == 4) {
    display.invertDisplay(true); delay(15); display.invertDisplay(false);
    if (inEditMode) adjustSetting(1);
    else { currentSettingPage++; if (currentSettingPage > 13) currentSettingPage = 0; } 
    refreshScreen();
  }
}
...
void adjustSetting(int d) {
  ...
  switch (currentSettingPage) {
    ...
    case 13: ok = trySet(soundType, d, 0, 2); break; 
  }
  if (!ok) shakeError();
}
...
// --- UPDATED: BULLETPROOF SCORING MATH ---
float getScore(float val, float aMin, float gMin, float gMax, float aMax) {
  // 1. If it's perfectly in the green
  if (val >= gMin && val <= gMax) return 1.0;          
  
  // 2. If it's completely out of bounds (deadly red)
  if (val <= aMin || val >= aMax) return 0.0;          
  
  // 3. Prevent divide by zero if thresholds get crossed accidentally
  if (gMin <= aMin || aMax <= gMax) return 0.0;
  
  // 4. Calculate proper sliding scale
  if (val < gMin) {
    return constrain((val - aMin) / (gMin - aMin), 0.0, 1.0); 
  }
  if (val > gMax) {
    return constrain((aMax - val) / (aMax - gMax), 0.0, 1.0); 
  }
...
void updateLED() {
  if (currentScreen == 4) {
    ...
    noTone(speakerPin); beepState = 0; 
    return;
  }

  int statusToEvaluate = 2; 
  ...
  if (currentScreen == 5 || currentScreen == 0) {
    ...
    if (health > 60) statusToEvaluate = 2;
    else if (health > 50) statusToEvaluate = 1;
    else statusToEvaluate = 0;
  } else {
    statusToEvaluate = checkStatus(currentScreen);
  }

  if (statusToEvaluate == 2) { 
    digitalWrite(redPin, HIGH); 
    digitalWrite(greenPin, LOW);
    noTone(speakerPin); beepState = 0; 
  } 
  else if (statusToEvaluate == 1) {  
    digitalWrite(redPin, HIGH); 
...
void drawFrame(int mode, int xo, bool showGraph) {
  ...
  if (mode == 5) { 
    ...
    // 3. Calculate where the data points lie on the axes
    int pxT = cx; 
    int pyT = cy - (sT * maxR);
    ...
    // 4. Draw the dynamic line connecting the points
    ...
    // 5. Draw the solid dots at the points
  ...
  if (mode == 4) {
    ...
    else { display.setCursor(xo + 15, 2); display.print(F("SYSTEM SETUP")); }
    ...
    switch (currentSettingPage) {
      ...
      case 13: settingName = "Alert Sound (0-2)"; settingVal = soundType; break; 
    }
    ...
    int thumbY = 14 + (currentSettingPage * 32 / 13); 
