...
// --- Global Variables ---
int currentScreen = 0; // 0: Intro, 1: Temp, 2: Hum, 3: Soil
float currentT = 0;
float currentH = 0;
int currentSoil = 0;
...
void setup() {
  ...
  // Draw the very first Intro Frame and sync LED
  display.clearDisplay();
  drawFrame(0, 0, true); 
  display.display();
  updateLED(); 
...
void loop() {
  ...
  if (gesture == APDS9960_LEFT || gesture == APDS9960_UP) {
    ...
    if (nextScreen > 3) nextScreen = 1; // Cycle through data screens, skipping intro
    animateScroll(currentScreen, nextScreen, 1);
    currentScreen = nextScreen;
  } 
  else if (gesture == APDS9960_RIGHT || gesture == APDS9960_DOWN) {
    int nextScreen = currentScreen - 1;
    // If we are at Intro (0) or Temp (1), swiping right goes to Soil (3)
  ...
  if (millis() - lastReadTime >= 5000) {
    ...
    // Sync the physical LED immediately after the screen draws
    updateLED(); 
  }
}
...
void updateLED() {
  ...
  else if (currentSoil <= 30) {
    // Yellow (Warning zone)
    digitalWrite(redPin, HIGH);
    digitalWrite(greenPin, HIGH);
    analogWrite(speakerPin, 0);
  } 
  else if (currentSoil <= 65) {
    // Green (Optimal Zone)
    digitalWrite(redPin, HIGH);
...
void drawFrame(int mode, int x_offset, bool showGraph) {
  ...
  if (mode == 0) {
    ...
    return; // No graph on intro screen
  }
  ...
  for (int i = 0; i < 20; i++) {
    int y;
    if (mode == 1) y = map(tempQueue[i], 15, 30, 62, 14);
    else if (mode == 2) y = map(humQueue[i], 0, 100, 62, 14);
    else if (mode == 3) y = map(soilQueue[i], 0, 100, 62, 14);
